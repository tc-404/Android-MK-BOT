...
function host.spawn_bg(cmd, key, long_running, title, cb) return __host_call("spawn_bg", cmd, key, long_running, title, cb) end
function host.run_ubuntu(script, key, long_running, title, cb) return __host_call("run_ubuntu", script, key, long_running, title, cb) end
function host.spawn_log(key) return __host_call("spawn_log", key) end
function host.spawn_running(key) return __host_call("spawn_running", key) end
...
-- image 支持 bind = reactive key: 文件内容变化时 bump rev 可强制 Image.file 重新加载
