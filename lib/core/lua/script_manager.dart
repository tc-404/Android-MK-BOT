...
import '../constants/scripts.dart' show ubuntuPath, isUbuntuInstalled;
import '../../ui/controllers/terminal_controller.dart';
import '../../ui/controllers/webview_tab_manager.dart';
...
class ScriptManager {
  ...
  static const String _defaultScriptsVersion = '36';

  /// 扩展沙盒库等关键脚本: 缺失会导致 require 失败, 启动时必须存在于 scriptsDir。
  static const List<String> _requiredScriptPaths = [
    'sandbox/audio_player.lua',
    'sandbox/audio_recorder.lua',
    'agent/main.lua',
    'agent/sandbox',
    'main.lua',
    'napcat.lua',
  ...
  /// 即使 AssetManifest 漏列, 也强制从 bundle 尝试释放的路径 (assets/ 前缀)。
  static const List<String> _bundledScriptAssets = [
    'assets/scripts/sandbox/audio_player.lua',
    'assets/scripts/sandbox/audio_recorder.lua',
    'assets/scripts/agent/main.lua',
    'assets/scripts/agent/sandbox',
    'assets/scripts/main.lua',
    'assets/scripts/napcat.lua',
    'assets/scripts/kakake.lua',
    'assets/scripts/agent.lua',
  ];
...
host.audio_player = ok and mod or function() return { play=function() end, stop=function() end, dispose=function() end, on=function() return 0 end, off=function() end } end
''', chunkName: 'audio_player_load');
  ...
  Future<void> _releaseDefaultScripts() async {
    ...
    final missingRequired = _missingRequiredScripts(dir.path);
    // 版本变化, 或关键扩展脚本缺失 (例如旧 APK 未打包 sandbox/) 时重新释放
    if (installedVer != _defaultScriptsVersion || missingRequired.isNotEmpty) {
      if (missingRequired.isNotEmpty) {
        debugPrint(
            '[ScriptManager] 检测到缺失脚本: ${missingRequired.join(", ")} → 重新释放');
      }
      try {
        ...
        for (final key in manifest.listAssets()) {
          ...
          await _copyBundledScriptAsset(key, '${dir.path}/$name');
          copied++;
        }
        await _ensureBundledCriticalScripts(dir.path);
  ...
  List<String> _missingRequiredScripts(String root) {
    final missing = <String>[];
    for (final rel in _requiredScriptPaths) {
      if (!File('$root/$rel').existsSync()) missing.add(rel);
    }
    return missing;
  }

  Future<void> _copyBundledScriptAsset(String assetKey, String outPath) async {
    final out = File(outPath);
  ...
  /// 兜底: 逐项从 bundle 写入关键脚本 (应对 pubspec 漏声明子目录导致 manifest 缺文件)。
  Future<void> _ensureBundledCriticalScripts(String root) async {
    for (final assetKey in _bundledScriptAssets) {
      final rel = assetKey.substring('assets/scripts/'.length);
      final outPath = '$root/$rel';
      if (File(outPath).existsSync()) continue;
      try {
        await _copyBundledScriptAsset(assetKey, outPath);
        debugPrint('[ScriptManager] 兜底释放: $rel');
      } catch (e) {
        debugPrint('[ScriptManager] 兜底释放失败 $assetKey: $e');
      }
    }
  }

  void _loadSandboxExtensions() {
    const code = r'''
local function _stub_player(name)
  name = name or "audio"
  return {
    name = name, playing = false, position = 0, duration = 0,
    play = function() end, pause = function() end, resume = function() end,
    stop = function() end, seek = function() end, setVolume = function() end,
    setLoop = function() end, on = function() return 0 end, off = function() end,
    dispose = function() end,
    enableMediaSession = function() end, updateMediaSession = function() end,
    disableMediaSession = function() end, onMediaButton = function() end,
  }
end
local ok, mod = pcall(require, "sandbox.audio_player")
if ok and mod then
  host.audio_player = mod
else
  host.error("sandbox.audio_player 加载失败: " .. tostring(mod))
  host.audio_player = _stub_player
end
local ok2, mod2 = pcall(require, "sandbox.audio_recorder")
if ok2 and mod2 then
  host.audio_recorder = mod2
else
  host.error("sandbox.audio_recorder 加载失败: " .. tostring(mod2))
  host.audio_recorder = function(name)
    return {
      name = name or "rec", recording = false, paused = false,
      start = function() end, stop = function() return nil end,
      pause = function() end, resume = function() end, discard = function() end,
      on = function() return 0 end, off = function() end, dispose = function() end,
    }
  end
end
''';
    try {
      _engine.doString(code, chunkName: 'sandbox_extensions_load');
    } catch (e) {
      debugPrint('[ScriptManager] 扩展沙盒模块加载异常: $e');
      LuaLog.instance.error('扩展沙盒模块加载异常: $e');
    }
  }
  ...
  Object? buildPage(String name, Map<String, dynamic> ctx) {
    ...
    try {
      final result = fn.call([ctx]);
      if (result == null) {
        const hint = 'Lua 回调执行失败 (详见 Lua 日志)';
        lastError = '页面「$name」$hint';
        LuaLog.instance.error(lastError!);
        return [
          {
            '__type': 'section',
            'title': '页面加载失败',
            'children': [
              {
                '__type': 'card',
                'children': [
                  {'__type': 'text', 'text': name, 'size': 16, 'weight': 'bold'},
                  {'__type': 'spacer', 'height': 8},
                  {'__type': 'text', 'text': hint, 'color': 'red', 'size': 13},
                ],
              },
            ],
          },
      ...
      lastError = '$e';
      LuaLog.instance.error('页面「$name」渲染失败: $e\n$st');
      return [
        {
          '__type': 'card',
          'title': '页面渲染失败',
          'children': [
            {'__type': 'text', 'text': '$e', 'color': 'red', 'size': 13},
            {'__type': 'spacer', 'height': 12},
            {
              '__type': 'text',
              'text': '可在设置 → Lua 日志查看详情',
              'color': 'grey',
              'size': 12,
            },
          ],
  ...
  void _registerHandlers() {
    ...
    e.registerHandler('ubuntu_installed', (_) => isUbuntuInstalled());
    ...
    e.registerHandler('webview_open', (a) {
      ...
      final url = WebViewTabManager.normalizeUrl(raw) ?? raw.trim();
    ...
    // 后台 spawn: 不创建终端 tab、不跳转; 日志用 host.spawn_log(key) 读取
    e.registerHandler('run_ubuntu', (a) {
      final hc = _home();
      if (hc == null || a.isEmpty) return null;
      final script = '${a[0]}';
      final key = a.length > 1 && a[1] != null ? '${a[1]}' : null;
      final longRunning = a.length > 2 && a[2] == true;
      var title = key ?? 'Ubuntu';
      LuaFunctionRef? cb;
      if (a.length > 3 && a[3] is LuaFunctionRef) {
        cb = a[3] as LuaFunctionRef;
    ...
    e.registerHandler('spawn_log', (a) {
      final hc = _home();
      if (hc == null || a.isEmpty) return '';
      return hc.spawnLog('${a[0]}');
    });
    e.registerHandler('spawn_running', (a) {
      final hc = _home();
      if (hc == null || a.isEmpty) return false;
      return hc.isSpawnRunning('${a[0]}');
    });
