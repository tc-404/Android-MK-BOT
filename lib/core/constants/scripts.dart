...
/// 与 install_ubuntu 中 NEED_INSTALL 判定一致; 额外要求就绪标记文件,
/// 避免仅有空目录时误判为已安装。
bool isUbuntuInstalled() {
  final marker = File('${RuntimeEnvir.homePath}/.ubuntu_ready');
  final dirsOk = Directory('$ubuntuPath/bin').existsSync() &&
      File('$ubuntuPath/usr/bin/env').existsSync() &&
      File('$ubuntuPath/usr/bin/apt-get').existsSync() &&
      Directory('$ubuntuPath/etc').existsSync();
  if (!dirsOk) {
...
void markUbuntuReady() {
  try {
    File('${RuntimeEnvir.homePath}/.ubuntu_ready').writeAsStringSync('ready');
  } catch (_) {}
}

/// install_ubuntu 各阶段 bump_progress 总步数 (Dart 侧换算进度条).
const int ubuntuInstallProgressSteps = 7;
...
install_ubuntu(){
  ...
  if [ ! -d "$UBUNTU_PATH/bin" ]; then
    ...
    NEED_INSTALL=1
  elif [ ! -f "$UBUNTU_PATH/usr/bin/apt-get" ]; then
    echo "[state] missing apt-get, force reinstall"
    NEED_INSTALL=1
  fi
  ...
  if [ "$NEED_INSTALL" -eq 1 ] || [ -z "$(ls -A $UBUNTU_PATH 2>/dev/null)" ]; then
    ...
    progress_echo "正在解压 Ubuntu 镜像 (约 1–3 分钟)..."
  ...
  progress_echo "正在配置软件源..."
  change_ubuntu_source
  bump_progress
  echo 'nameserver 8.8.8.8' > $UBUNTU_PATH/etc/resolv.conf
  bump_progress
  progress_echo "Ubuntu 环境就绪"
  echo "ready" > "$HOME_PATH/.ubuntu_ready"
}
''';
