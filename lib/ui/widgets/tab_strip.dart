...
/// 标签栏内 tab 的对齐方式。
enum TabStripAlign { start, distribute }
...
class TabStrip extends StatelessWidget {
  ...
  /// [distribute] 时 tab 在可用宽度内居中平均分布 (适合页内 2–4 个固定标签)。
  final TabStripAlign align;
  ...
  const TabStrip({
    ...
    this.opacity = 0.6,
    this.blur = 18,
    this.align = TabStripAlign.start,
  });
  ...
  Widget _buildList(BuildContext context) {
    ...
    if (align == TabStripAlign.distribute && onReorder == null) {
      return Row(
        mainAxisAlignment: MainAxisAlignment.spaceEvenly,
        children: [
          for (var i = 0; i < items.length; i++) _buildEntry(context, i),
        ],
      );
    }
    if (onReorder == null) {
