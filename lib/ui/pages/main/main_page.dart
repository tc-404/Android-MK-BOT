...
class _MainPageState extends State<MainPage> {
  ...
  void initState() {
    super.initState();
    // 进入主界面时清除安装遮罩状态 (遮罩仅在启动页使用)
    homeController.ubuntuBootstrapActive.value = false;
    ...
    return Obx(
      () => BubbleBackground(
        ...
        child: Scaffold(
          ...
          bottomNavigationBar:
              _showSettings ? null : _buildBottomNav(context),
        ),
      ),
    );
  }
  ...
  Widget _pageFor(Map<String, dynamic> tab) {
          ...
          GlassAppBar(
            ...
            titleSuffix: _isHome(page)
                ? IconButton(
                    ...
                    onPressed: () =>
                        ScriptManager.instance.reloadWithGuard(silent: true),
                  )
                : null,
            actions: [
              ..._buildLuaActions(),
    ...
    return SafeArea(
      bottom: false,
      child: SizedBox.expand(child: LuaPage(pageName: name)),
    );
  }
