  ...
  Widget build(BuildContext context) {
    return TickerMode(
      ...
      child: Obx(() {
        ...
        if (desc is List && desc.isEmpty) {
          return _missing(context);
        }
        if (desc is Map &&
            ('${desc['__type']}' == 'tabs' || desc['fill'] == true)) {
          return SizedBox.expand(child: renderer.build(context, desc));
        }
        if (desc is List) {
          return SingleChildScrollView(
            key: ValueKey('lua_scroll_${widget.pageName}'),
              ...
              children: [
                for (var i = 0; i < desc.length; i++)
                  Padding(
                    ...
                    child: renderer.build(context, desc[i]),
                  ),
              ],
            ),
          );
        ...
        return SingleChildScrollView(
          key: ValueKey('lua_root_${widget.pageName}'),
          padding: const EdgeInsets.fromLTRB(16, 8, 16, 24),
          child: renderer.buildRoot(context, desc),
        );
      }),
    );
  }
      ...
      child: GlassPanel(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              '脚本可能未同步到最新版本。请尝试重新加载脚本, 或在设置中查看 Lua 日志。',
              style: Theme.of(context)
                  .textTheme
                  .bodyMedium
...
class LuaRenderer {
  ...
  Widget _image(Map node) {
    ...
    if (bind is String) {
      ...
      return ValueListenableBuilder<Object?>(
        valueListenable: n,
        builder: (_, rev, __) => _imageFile(path, w, h, rev),
      );
    }
    return _imageFile(path, w, h, null);
  }

  Widget _imageFile(String path, double? w, double? h, Object? cacheRev) {
  ...
  Widget _button(BuildContext context, Map node) {
    ...
    final errorColor = danger ? Theme.of(context).colorScheme.error : null;
    switch (variant) {
      case 'tonal':
        return FilledButton.tonal(
          style: errorColor == null
              ? null
              : FilledButton.styleFrom(foregroundColor: errorColor),
          onPressed: onTap,
          child: child,
        );
      case 'outlined':
  ...
  Widget _tabs(BuildContext context, Map node) {
    ...
    final tabAlign = '${node['align'] ?? 'start'}' == 'distribute'
        ? TabStripAlign.distribute
        : TabStripAlign.start;
