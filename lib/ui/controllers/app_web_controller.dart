...
class AppWeb {
  ...
  static AppWebController create(
    String url, {
    void Function(String url)? onInWebViewUrl,
  }) =>
      SystemWebController(url, onInWebViewUrl: onInWebViewUrl);
  ...
  static const String pagePatchJs = '''
    (function() {
      if (window.__wvPagePatched) return;
      window.__wvPagePatched = true;
      ...
      document.addEventListener('click', function(e) {
        ...
        try {
          ...
          var same = u.origin === window.location.origin;
...
class SystemWebController implements AppWebController {
  ...
  final void Function(String url)? _onInWebViewUrl;

  SystemWebController(
    String url, {
    void Function(String url)? onInWebViewUrl,
  })  : _interceptExternal = AppWeb.isLocalUrl(url),
        _onInWebViewUrl = onInWebViewUrl {
    ...
    _c.setNavigationDelegate(
      NavigationDelegate(
        ...
        onUrlChange: (change) {
          final u = change.url;
          if (u != null && u.isNotEmpty && u != 'about:blank') {
            _onInWebViewUrl?.call(u);
          }
        },
        onPageFinished: (_) {
          _c.runJavaScript(AppWeb.pagePatchJs);
        },
